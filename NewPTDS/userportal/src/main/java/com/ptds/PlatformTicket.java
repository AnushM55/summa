...
public class PlatformTicket extends JFrame {
    ...
    private void PersonTextFieldActionPerformed(ActionEvent evt) {
     ...
     public void printoTag() throws IOException {
         ...
         float radius = 200;
         Ellipse2D.Float circle = new Ellipse2D.Float(centerX - radius, centerY - radius, 2 * radius, 2 * radius);

         // Create clipping path
         contentStream.saveGraphicsState();
         contentStream.addRect(circle.x, circle.y, circle.width, circle.height);
         contentStream.clip();

         // Fill the circle with a background color (optional)
//        contentStream.setNonStrokingColor(Color.WHITE); // white background
        
  //       contentStream.fill();
         // Set font and begin text inside the circle
         contentStream.setFont(new PDType1Font(font_name_3v.HELVETICA_BOLD), 20);
        contentStream.beginText();
         contentStream.setLeading(14.5f);
         // Calculate the text position
         contentStream.newLineAtOffset(centerX - radius + 20, centerY + radius - 40);
         ...
         contentStream.newLine();
         contentStream.showText("Persons: " + PersonTextField.getText());
         contentStream.newLine();
         contentStream.showText("Platform No: " + PlatformNoTextField.getText());
         contentStream.newLine();
         contentStream.showText("Amount: " + AmountTextField.getText());
         contentStream.newLine();
         contentStream.setFont(new PDType1Font(font_name_3v.COURIER), 14);
        contentStream.showText("Valid for 2 hours only!!");

         contentStream.endText();

         // Restore the graphics state to stop clipping
        contentStream.restoreGraphicsState();
         contentStream.close();
         String fileName = "Tag_PlatformTicket_" + TicketIdTextField.getText() + ".pdf";
    ...
    public void printToPDF() {
        try {
            ...
            pstmt.setInt(2, Integer.parseInt(PersonTextField.getText()));
            pstmt.setInt(3, Integer.parseInt(PlatformNoTextField.getText()));
            pstmt.setInt(4, Integer.parseInt(AmountTextField.getText()));
            ...
            conn.close();
                ...
                JOptionPane.showMessageDialog(null, "Error occurred while connecting to database: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            }
            try{
...
/* 
            ...
            String[] lines = {
                "   PLATFORM TICKET",
                "Ticket ID: " + TicketIdTextField.getText(),
                "Date: " + DateTextField.getText(),
                "Person(s): " + PersonTextField.getText(),
                "Platform Number: " + PlatformNoTextField.getText(),
                "Amount: Rs." + AmountTextField.getText(),
            ...
            float borderHeight = (lines.length * leading) + 2 * borderPadding;
            ...
            JOptionPane.showMessageDialog(null, "Error occurred while creating PDF: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(null, "Error occurred while connecting to database: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        */
    }
